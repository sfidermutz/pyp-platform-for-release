...
type ModuleRecord = {
  ...
  is_demo: boolean;
  module_families?: ModuleFamily; // normalized to single object
};
...
export default function CoinsPage() {
  ...
  useEffect(() => {
    const token =
      typeof window !== 'undefined' ? localStorage.getItem('pyp_token') : null;
    if (!token) {
      router.push('/');
      return;
    }
  ...
  async function fetchModules() {
    ...
    const { data, error } = await supabase
      .from('modules')
      .select(
        `id, name, description, shelf_position, is_demo, module_families ( name, code )`
      )
      .eq('is_demo', true)
    ...
    // Normalize module_families: Supabase returns an array, we store a single object
    const normalized: ModuleRecord[] = (data ?? []).map((m: any) => {
      ...
      if (Array.isArray(fam)) {
        familyObj = fam.length
          ? { name: fam[0].name, code: fam[0].code }
          : null;
    ...
    try {
      const session_id =
        typeof window !== 'undefined'
          ? localStorage.getItem('pyp_session_id')
          : null;
  ...
  function onSelectModule(m: ModuleRecord) {
    ...
    logEvent({
      ...
      payload: { module_id: m.id, module_name: m.name },
    });
  }
  ...
  async function onEnterModule() {
    if (!selected) return;
    await logEvent({
      event_type: 'enter_module',
      payload: { module_id: selected.id },
    });
    ...
    <main className="min-h-screen bg-black text-white px-6 py-12">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl tracking-widest font-bold mt-2">
            CHALLENGE COINS
          </h1>
        </div>
        ...
        <div className="bg-[#0b0f14] border border-[#202933] rounded-3xl p-8 shadow-inner">
          <div className="grid grid-cols-6 gap-8">
            {modules.map((m) => {
              const ready = true; // demo simple
              return (
                <div key={m.id} className="text-center">
                  <button
                    onClick={() => ready && onSelectModule(m)}
                    className={`w-36 h-36 rounded-full mx-auto border-2 ${
                      selected?.id === m.id
                        ? 'ring-4 ring-sky-500'
                        : 'border-slate-700'
                    } flex items-center justify-center bg-gradient-to-b from-[#0f1720] to-transparent`}
                  >
                    <span className="text-xl font-semibold">
                      {m.shelf_position ?? '?'}
                    </span>
                  </button>
                  <div className="mt-3 text-xs font-semibold tracking-wider">
                    {m.name}
                  </div>
                  <div
            ...
            <div className="relative bg-[#0b0f12] rounded-xl border border-slate-800 p-6 max-w-xl w-full z-10">
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded bg-sky-500 text-black font-bold"
                >
                  Enter Module
                </button>
              </div>
